...
def main():
    ...
    data_dir = validation_dir / "dashboard"  # CSVs are in dashboard folder
    
    # Find dashboard YAML files
    dashboard_files = sorted(validation_dir.glob("dashboard/dashboard-*.yaml"))
    
    if not dashboard_files:
        st.error("No dashboard configuration files found!")
        st.info(f"Looking in: {validation_dir / 'dashboard'}")
        return
