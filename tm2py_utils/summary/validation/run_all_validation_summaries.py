...
class RunConfig(BaseModel):
    ...
    dataset_order: Optional[List[str]] = None  # Order of datasets in charts (by display_name or name)
...
class DataLoader:
    ...
    def __init__(self, file_specs: CTRAMPFileSpecs, column_mapping: ColumnMapping = None, data_model_path: Optional[Path] = None, binning_specs: Optional[Dict[str, BinningSpec]] = None):
        ...
        if data_model_path is None:
            ...
            data_model_path = Path(__file__).parent.parent / "ctramp_data_model.yaml"
...
class SummaryGenerator:
    ...
    def __init__(self, config: SummaryConfig, data_model=None, dataset_display_names: Dict[str, str] = None):
        self.config = config
        self.data_model = data_model
        self.dataset_display_names = dataset_display_names or {}
...
def _combine_multi_run_summaries(summaries: Dict[str, pd.DataFrame], dataset_order: Optional[List[str]] = None) -> Dict[str, pd.DataFrame]:
    """
    ...
    for base_name, dfs in grouped.items():
        if len(dfs) == 1:
            ...
            # Sort by dataset column based on specified order or alphabetically
            if 'dataset' in combined.columns:
                ...
                combined = combined.sort_values('dataset')
            result[base_name] = combined
    
    return result


def save_summaries(summaries: Dict[str, pd.DataFrame], output_dir: Path, 
                  output_config: OutputConfig = None, generate_dashboard: bool = True,
                  dataset_order: Optional[List[str]] = None):
    """Save all summary tables to CSV files and optionally generate dashboards.
    ...
    Args:
        ...
        generate_dashboard: Whether to generate dashboard files (default: True)
        dataset_order: Optional list specifying order of datasets in charts
    """
    ...
    if generate_dashboard:
        ...
        combined_summaries = _combine_multi_run_summaries(summaries, dataset_order)
...
def main():
    ...
    try:
        ...
        # Suggest running dashboard if dashboards were generated
        if not args.no_dashboard:
            dashboard_dir = config.output_directory / "dashboard"
            if dashboard_dir.exists():
                logger.info("\n📊 To view the dashboards, run:")
                logger.info(f"   python -m tm2py_utils.summary.validation.run_dashboard")
